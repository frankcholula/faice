...
def evaluate(config, epoch, pipeline, timesteps):
...
def calculate_inception_score(config, pipeline, timesteps, test_dataloader, device=None):
    if device is None:
...
def calculate_fid_score(config, pipeline, timesteps, test_dataloader, device=None, save=True):
