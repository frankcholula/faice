        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if isinstance(noise_scheduler, CMStochasticIterativeScheduler):
                ...
                noisy_images = noise_scheduler.add_noise(
                    clean_images, noise, init_timesteps
                )
            else:
            ...
            with accelerator.accumulate(model):
                ...
                if isinstance(noise_scheduler, CMStochasticIterativeScheduler):
                    ...
                    weights = append_dims(
                        get_weightings(
                            weight_schedule, snrs, noise_scheduler.config.sigma_data
                        ),
                        clean_images.ndim,
                    )
...
def convert_sigma(noise_scheduler, original_samples, timesteps):
    ...
    if original_samples.device.type == "mps" and torch.is_floating_point(timesteps):
        ...
        schedule_timesteps = noise_scheduler.timesteps.to(
    ...
    if noise_scheduler.begin_index is None:
        step_indices = [
            noise_scheduler.index_for_timestep(t, schedule_timesteps) for t in timesteps
        ]
